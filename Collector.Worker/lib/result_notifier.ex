defmodule Collector.ResultNotifier do
  use GenServer
  ...
  defp notify_new_results(user_sources_data) do
    Phoenix.PubSub.direct_broadcast(:"ui@127.0.0.1", Collector.PubSub, "result_notifier", {:updated_user_sources, user_sources_data})
  end
  ...
  def handle_cast(operation, state) do
    case operation do
      ...
      {:send_result, user_source_update_raw} -> if Enum.member?(Node.list(), :"ui@127.0.0.1") do
						  ...
						  notify_new_results(user_sources_data)
						  IO.inspect "Broadcast user_sources_data: #{inspect user_sources_data}"
						else
