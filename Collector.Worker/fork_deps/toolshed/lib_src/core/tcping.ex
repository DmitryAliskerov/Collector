defmodule Toolshed.Core.Tcping do
  ...
  * `:port` - Which TCP port to try (defaults to 80)
  ...
  def tcping_result_ms(address, options \\ []) do
    do_tcping_result_ms(address, options)
  end
  ...
  defp do_tcping_result_ms(address, options) do
    case resolve_addr(address) do
      {:ok, ip} -> tcping_ip_result_ms(address, ip, options)
      {:error} -> 0
    end
  end
  ...
  defp tcping_ip_result_ms(address, ip, options) do
    port = Keyword.get(options, :port, 80)

    case try_connect(ip, port, options) do
      {:ok, micros} -> round(micros / 1000)
      {:error} -> 0
    end
  end
